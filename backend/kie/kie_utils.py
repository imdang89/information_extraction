...
def make_text_encode(text):# add them ky tu moi (neu co) va creat array ky tu
    text_encode = []
    for t in text.upper():# xet word viet hoa
        if t not in cf.alphabet:
            text_encode.append(cf.alphabet.index(" "))#tao list ky tu
        else:
            text_encode.append(cf.alphabet.index(t))
    return np.array(text_encode)# return ra 1 mang


def prepare_data(cells, text_key="vietocr_text"):#cell duoi dang json chua thong tin ocr,poly ; chuan bi ocr va poly(vi tri ocr)
    texts = []# text
    text_lengths = []# len(text)
    text_all=[]
    polys = []# bbox dang polygon
    for cell in cells:
        text = cell[text_key]
        text_encode = make_text_encode(text)# add and creat
        text_lengths.append(text_encode.shape[0])# so ky tu trong array
        texts.append(text_encode)
        text_all.append(text)


        poly = copy.deepcopy(cell["poly"].tolist())# tao 1 ban sao dang list
    ...
    return texts, text_lengths, polys,text_all
...
def prepare_graph(cells):
    texts, text_lengths, boxes,text_all = prepare_data(cells)
...
def run_predict(gcn_net, merged_cells, device="cpu"):
    ...
    return batch_scores, boxes,text_all
